"""
MoE — CK kernel injection for ALL E<=64 cases including d=2048.
Key changes from best_kernels:
- d=2048 now uses 64x32 kernel (256x32 crashed before, 64x32 works)
- Removed inter_dim < 2048 guard
"""
import torch
import functools
...
def _patch():
    ...
    # Inject CK kernels for ALL E<=64 cases
        ...
        if (expert <= 64 and q_type == QuantType.per_1x32
            ...
            try:
                ...
                if not kw.get('kernelName', ''):
                    est_m = token * topk // expert
                    # For d>=2048: only use 64x32 kernel (256x32 crashes)
                    if inter_dim >= 2048:
                        kn1 = STAGE1_64
                    else:

...
#!POPCORN gpu MI355X
"""
MoE — Direct call to fused_moe_2stages, bypassing fused_moe wrapper.
Eliminates Python overhead from the top-level dispatch.
Also: pre-allocate output buffer, minimize Python overhead.
"""
import torch
import functools
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if out_key not in _out_cache:
        _out_cache[out_key] = torch.empty((M, model_dim), dtype=torch.bfloat16,
    ...
    fm.fused_moe_2stages(
        ...
        num_valid_ids, moe_buf, isG1U1, block_m,

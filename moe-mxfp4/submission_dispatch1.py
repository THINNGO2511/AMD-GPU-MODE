...
#!POPCORN gpu MI355X
"""
MoE — Test dispatch_policy=1 for moe_sorting.
The default dispatch_policy=0 might not be optimal.
dispatch_policy=1 might use a different token distribution strategy.
"""
import torch
import functools
...
def _patch():
    ...
    # Patch moe_sorting to use dispatch_policy=1
    orig_moe_sorting = fm.moe_sorting
    def new_moe_sorting(topk_ids, topk_weights, num_experts, model_dim,
                        moebuf_dtype, block_size=32, expert_mask=None,
                        num_local_tokens=None, dispatch_policy=0):
        return orig_moe_sorting(topk_ids, topk_weights, num_experts, model_dim,
                                moebuf_dtype, block_size, expert_mask,
                                num_local_tokens, dispatch_policy=1)
    fm.moe_sorting = new_moe_sorting

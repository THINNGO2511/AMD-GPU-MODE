"""
MoE Experiment: Force large-tile CK kernels for d=2048.
Key insight: The fc0c54bb commit has 256x128x128x128 tiles for d=2048,
but the runner CSV doesn't match E=33 topk=9. We need to unconditionally
override the heuristic for d>=2048 with these large tiles.

Also: override the kernelName check that prevented v1 from firing.
"""
import torch
import functools
...
# Large tile kernels for d=2048 (from fc0c54bb commit for E=32 topk=8)
...
def _patch():
    ...
    # Custom block size selection
    def custom_bsm(t, k, e, d):
        est_m = t * k // e
        if d >= 2048:
            return 128 if est_m >= 100 else 64
        if e <= 64:
            return 32 if est_m < 50 else 64
        return fm.get_block_size_M.__wrapped__(t, k, e, d) if hasattr(fm.get_block_size_M, '__wrapped__') else 32
    fm.get_block_size_M = custom_bsm
    ...
    # Override get_2stage_cfgs to inject kernels for ALL shapes including d=2048
    ...
    def new_get_2stage(token, model_dim, inter_dim, expert, topk,
        ...
        if expert <= 64 and q_type == QuantType.per_1x32 and not result.run_1stage:
            ...
            if inter_dim >= 2048:
                # Force large-tile kernels for d=2048
                # Use 128 block_m for bs=512 (est_m ~140), 64 for smaller
                ...
                # d<2048: use small/medium tile kernels
                try:

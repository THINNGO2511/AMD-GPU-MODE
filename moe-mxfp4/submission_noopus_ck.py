...
#!POPCORN gpu MI355X
"""
MoE — CK kernel injection WITHOUT opus sorting.
Leaderboard shows competitor "noopus" at 144us (rank 14).
Opus sorting may add overhead without benefit for some cases.
Also try: use_nt=False for ALL experts (not just E<=64).
"""
import torch
import functools
...
def _patch():
    ...
    # Force use_nt=False for ALL cases (not just E<=64)
    ...
    # Explicitly DISABLE opus sorting
    try:

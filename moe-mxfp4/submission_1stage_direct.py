...
#!POPCORN gpu MI355X
"""
MoE — Call fused_moe_1stage directly for E<=64 cases.
The 1-stage ASM kernel fuses quant+sort+gemm1+silu+gemm2 in one kernel,
eliminating inter-stage quantization overhead.
For E=257, keep the 2-stage path with CSV configs.
"""
import torch
import functools
...
def _patch():
    ...
    # CK injection for 2-stage fallback (E<=64 d<2048)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Try 1-stage for E<=64 d<2048 cases (d>=2048 fails accuracy in 1-stage)
    if E <= 64 and inter_dim < 2048:
        try:
            ...
            fm.fused_moe_1stage(
                hidden_states,
                gate_up_weight_shuffled, down_weight_shuffled,
                topk, sorted_ids, sorted_weights, sorted_expert_ids,
                num_valid_ids, moe_buf, isG1U1,
            ...
            print(f"[1STAGE ERR] {e}, falling back to 2-stage")

    # Fallback to 2-stage for all cases
    return fused_moe(
        hidden_states,
        gate_up_weight_shuffled, down_weight_shuffled,

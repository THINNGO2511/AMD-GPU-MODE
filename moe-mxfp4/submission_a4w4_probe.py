...
#!POPCORN gpu MI355X
"""
MoE — Read the moe_op_gemm_a4w4.py launcher code (lines 400-587).
This has a matmul_ogs-style launcher that handles grid/buffer setup.
"""
import torch
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if not _probed:
        _probed = True
        # Read the launcher part of moe_op_gemm_a4w4.py (lines 400-587)
        path = "/home/runner/aiter/aiter/ops/triton/_triton_kernels/moe/moe_op_gemm_a4w4.py"
        with open(path) as f:
            lines = f.readlines()
        print(f"[A4W4] Lines 400-587:")
        for i in range(400, min(587, len(lines))):

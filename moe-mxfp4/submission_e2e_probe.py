...
#!POPCORN gpu MI355X
"""
MoE — Probe end-to-end MoE kernel + search for new aiter APIs.
"""
import torch
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if not _probed:
        ...
        # 1. Search for new/undiscovered aiter MoE functions
        print("[NEW] aiter MoE/fused functions:")
        for name in sorted(dir(aiter)):
            if any(k in name.lower() for k in ['moe', 'fused', 'expert', 'fmoe']):
        ...
        # 2. Read moe_op_e2e.py header to understand the e2e kernel
        try:
            path = "/home/runner/aiter/aiter/ops/triton/_triton_kernels/moe/moe_op_e2e.py"
            with open(path) as f:
                lines = f.readlines()
            print(f"\n[E2E] {len(lines)} lines, functions:")
            for i, line in enumerate(lines):
                if line.strip().startswith('def ') or line.strip().startswith('@triton'):
                    print(f"  {i}: {line.rstrip()}")
            # Read the last 100 lines (launcher code)
            print(f"\n[E2E] Launcher (last 100 lines):")
            for i in range(max(0, len(lines)-100), len(lines)):
                print(f"  {i}: {lines[i].rstrip()}")
        except Exception as e:
            print(f"[E2E] error: {e}")

        # 3. Check for newer fused_moe versions or APIs
        for name in ['fused_moe_e2e', 'fused_moe_triton', 'triton_fused_moe',
                      'fused_moe_v2', 'fused_moe_fp4', 'fused_moe_mxfp4']:
        ...
        # 4. Check fused_moe_1stage_dict for available 1-stage configs
        try:
            d = fm.fused_moe_1stage_dict
            print(f"\n[1STAGE] fused_moe_1stage_dict keys: {list(d.keys())}")
            for arch, configs in d.items():
                print(f"  {arch}: {len(configs)} configs")
                for cfg in list(configs)[:5]:
                    print(f"    {cfg}")
        except Exception as e:
            print(f"[1STAGE] error: {e}")

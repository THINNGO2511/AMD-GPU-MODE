...
#!POPCORN gpu MI355X
"""
MLA — safe_fast with reduced num_kv_splits for kv=1024.

mla_safe_fast scored 41.5μs. bs=256 kv=1024 at 91.5μs is the bottleneck.
For that shape: total_kv=262144, currently uses num_kv_splits=16.
16-way split+reduce is expensive. Try 8-way.

Only change from mla_safe_fast: num_kv_splits capping.
Same Triton kernels, same pg1/pg8 routing, same skip-amax.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # KEY CHANGE: cap kv_splits at 8 for kv=1024
    # bs=256 kv=1024 was using 16 splits → 8 saves reduction overhead
    if batch_size <= 4:
        num_kv_splits = 4
    else:
        num_kv_splits = 8  # was 8 for small, 16 for large. Now 8 always.

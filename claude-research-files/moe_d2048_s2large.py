...
#!POPCORN gpu MI355X
"""
MoE — S1_64 + larger S2 (64x128x128x128) for d=2048.

Stage2 GEMM outputs 7168 features (hidden_dim). With S2_V1 (BN=32),
that's 224 tile columns. S2_64x128 (BN=128) reduces to 56 columns —
4x fewer tile launches, better throughput.

This kernel exists but was never tried with block_m=32.
...
# Larger stage2 for d=2048 (BN=128 instead of BN=32)
S2_64x128 = "moe_ck2stages_gemm2_64x128x128x128_1x1_MulABScaleExpertWeightShuffled_v3_Nswizzle0_Quant3_MulRoutedWeight1_FP4X2_FP4X2_B16"
    ...
    def new_g2s(token, model_dim, inter_dim, expert, topk,
        ...
        if expert <= 64 and q_type == QuantType.per_1x32 and not r.run_1stage:
            ...
            if not kw.get('kernelName', ''):
                est_m = token * topk // expert
                if inter_dim >= 2048:
                    # d=2048: S1_64 + S2_64x128 (large N-tile for 7168 output)
                    return fm.MOEMetadata(
                        ...
                        functools.partial(aiter.ck_moe_stage2_fwd,
                            kernelName=S2_64x128, activation=activation,

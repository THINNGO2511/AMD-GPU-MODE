...
#!POPCORN gpu MI355X
"""
MLA — qseqlen2 kernel dispatch.

RUNNER DISCOVERY: mla_a8w8_qh16_qseqlen2_gqaratio16_ps.co exists but
we've NEVER dispatched to it. We always pass max_seqlen_q=1 (qseqlen1).

qseqlen2 processes 2 queries simultaneously — could 2x throughput
for large batch sizes by amortizing memory access overhead.

To dispatch: set max_seqlen_q=2 in mla_decode_fwd.
This requires reshaping qo_indptr to reflect 2 queries per batch element.

josusanmartin's MLA at 19.5μs is 2x faster than next competitor.
qseqlen2 or qseqlen4 could explain this.

Try both qseqlen=2 for all shapes and qseqlen=4 for large batches.
"""
import torch
import triton
...
def _build_meta(batch_size, kv_seq_len, q_seq_len, max_seqlen_q, nq, nkv,
    ...
    info = get_mla_metadata_info_v1(
        batch_size, max_seqlen_q, nq, dtype_q, FP8_DTYPE,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Choose qseqlen based on batch size
    # qseqlen2: batch q in pairs → half the "batch" count, 2 queries each
    # This only works if q_seq_len allows it (must be 1, we reshape to 2)
    if batch_size >= 64:
        max_seqlen_q = 4  # Try qseqlen4 for large batches
        new_batch = batch_size // 4
        qo_indptr_new = torch.arange(0, new_batch + 1, dtype=torch.int32, device="cuda") * 4
        kv_indptr_new = torch.arange(0, new_batch + 1, dtype=torch.int32, device="cuda") * (kv_seq_len * 4)
    elif batch_size >= 16:
        max_seqlen_q = 2  # Try qseqlen2 for medium batches
        new_batch = batch_size // 2
        qo_indptr_new = torch.arange(0, new_batch + 1, dtype=torch.int32, device="cuda") * 2
        kv_indptr_new = torch.arange(0, new_batch + 1, dtype=torch.int32, device="cuda") * (kv_seq_len * 2)
    else:
        max_seqlen_q = 1  # Keep qseqlen1 for small batches
    ...
    cache_key = (new_batch, kv_seq_len, num_kv_splits, page_size, max_seqlen_q)
    if cache_key not in _meta_cache:
        _meta_cache[cache_key] = _build_meta(
            new_batch, kv_seq_len, q_seq_len, max_seqlen_q, nq, nkv,
            num_kv_splits, page_size, dtype_q, qo_indptr_new, kv_indptr_new)
    ...
    alloc_key = ("qseq", q.shape[0], nq, dv, dq)
    ...
    if _call <= 2:
        print(f"[MLA] bs={batch_size} kv={kv_seq_len} max_seqlen_q={max_seqlen_q} "
              f"new_batch={new_batch} splits={num_kv_splits} ps={ps}", flush=True)

    mla_decode_fwd(
        q_fp8_flat.view(q.shape[0], nq, dq), kv_buffer_4d, o,
        qo_indptr_new, kv_indptr_pages, kv_indices, kv_last_page_len,
        max_seqlen_q, page_size=ps, nhead_kv=nkv,

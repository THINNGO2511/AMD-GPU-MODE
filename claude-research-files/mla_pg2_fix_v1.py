...
#!POPCORN gpu MI355X
"""
MLA — pg2 fix attempt: DON'T divide kv_indptr by page_size.

DISCOVERY: The reference code passes kv_indptr UNCHANGED (token-level)
to both get_mla_metadata_v1 and mla_decode_fwd. It never divides by
page_size. The function internally handles paging.

Our pg2 code pre-divides: kv_indptr_pages = kv_indptr // page_size.
This might be WRONG — causing the ~4% mismatch that fails secret seeds.

This experiment: pass ORIGINAL kv_indptr (tokens, not pages) with
page_size=2. Let the internal code handle page division.

Also try kv_granularity=max(page_size, 16)=16 (matching reference formula)
instead of our max(1, 16//page_size)=8.

If this fixes accuracy: pg2 gives ~55μs for bs=256 kv=1024 (vs 69μs pg1).
Combined with fp8 Q + skip-amax: could hit 32-33μs geomean.
"""
import torch
import triton
...
def _build_meta(batch_size, kv_seq_len, q_seq_len, nq, nkv,
    ...
    # FIX: compute kv_last_page_len from original token counts
    ...
    # FIX: kv_granularity = max(page_size, 16) — matching reference formula!
    # NOT max(1, 16 // page_size) which we were using before
    ...
    # FIX: pass ORIGINAL kv_indptr (token-level), NOT divided by page_size
    # Also pass kv_last_page_len computed from original token counts
    get_mla_metadata_v1(
        qo_indptr, kv_indptr, kv_last_page_len,  # <-- kv_indptr NOT divided
        nq // nkv, nkv, True,
    ...
    return (wm, wi, wis, ri, rfm, rpm, kv_indices, kv_last_page_len, kv_indptr, page_size)
    # NOTE: returns original kv_indptr, NOT divided
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # pg2 for kv<=1024 (testing the fix!), pg8 for kv>=8192
    if kv_seq_len <= 1024:
        page_size = 2
    else:
        page_size = 8
    ...
    (wm, wi, wis, ri, rfm, rpm,
     kv_indices, kv_last_page_len, kv_indptr_out, ps) = _meta_cache[cache_key]

    kv_buffer_fp8, kv_scale = kv_data["fp8"]
    kv_buffer_4d = kv_buffer_fp8.view(-1, ps, nkv, kv_buffer_fp8.shape[-1])

    alloc_key = ("fp8pg2fix", q.shape[0], nq, dv, dq)
    ...
    # FIX: pass original kv_indptr (token-level) to mla_decode_fwd too
    mla_decode_fwd(
        q_fp8_flat.view(q.shape[0], nq, dq), kv_buffer_4d, o,
        qo_indptr, kv_indptr_out, kv_indices, kv_last_page_len,  # <-- NOT divided

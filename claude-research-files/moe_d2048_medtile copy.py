...
#!POPCORN gpu MI355X
"""
MoE — Current best (163μs) + 256x32 tile injection for d=2048.

Findings from this round:
- CK injection IS helping (188μs without vs 163μs with)
- CK kernels CRASH with block_m=128 (library default for large est_m)
- CK kernels REQUIRE block_m=32 or 64

d=2048 at 333μs dominates the geomean. Try 256x32x128x128 tiles
(proven for E=257) with block_m=64 (safe for CK kernels).

Everything else identical to submission_optimized_v2 (the 163μs version).
"""
import torch
import functools
...
# Medium stage2 for d=2048 (from dsv3 CSV, proven for E=257)
...
def _patch():
    ...
    # Keep SAME block_m logic as submission_optimized_v2
    ...
    def new_bsm(token, topk, expert, inter_dim):
        if expert <= 64:
            ...
            if est_m < 50:
                ...
                return 128  # keep default for d=2048 WITHOUT CK injection
            else:
                return 64
    ...
    def new_g2s(token, model_dim, inter_dim, expert, topk,
        ...
        if expert <= 64 and q_type == QuantType.per_1x32 and not r.run_1stage:
            ...
            if not kw.get('kernelName', ''):
                est_m = token * topk // expert
                if inter_dim >= 2048:
                    # NEW: d=2048 injection with medium tiles + block_m=64
                    return fm.MOEMetadata(
                        ...
                        64, 0, False)  # block_m=64 (CK-safe)
                else:
                    # d<2048: same as current best

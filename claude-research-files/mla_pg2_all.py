...
#!POPCORN gpu MI355X
"""
MLA — pg2 for ALL shapes (kv=1024 AND kv=8192).
Competitors at 33μs use pg2 everywhere.

Key question: was pg2 for kv=1024 ever retried with the CORRECTED
kv_granularity = max(1, 16 // page_size) = 8?
The dead end from Session 10 used the OLD formula (max(page_size, 16) = 16).

This uses:
- pg2 + bf16 Q for kv=1024 (skip fp8 quant overhead for small KV)
- pg2 + fp8 Q for kv=8192 (fp8 Q saves bandwidth for large KV)
- Correct kv_granularity = 8 throughout
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Choose Q dtype based on KV size (bandwidth tradeoff)
    if kv_seq_len <= 1024:
        dtype_q = BF16
        use_fp8_q = False
    else:
    ...
    if use_fp8_q:
        alloc_key = ("fp8_pg2all", q.shape[0], nq, dv, dq)
        ...
        alloc_key = ("bf16_pg2all", q.shape[0], nq, dv)
        if alloc_key not in _alloc_cache:

...
#!POPCORN gpu MI355X
"""
MoE — S1_64 for d=2048 (small tiles for small est_m).

Current d=2048 uses default kernels (no injection). 333μs.
Previous attempt used S1_256 (BM=256) but est_m=139.
That means the 256-token tile is 46% wasted occupancy.

S1_64 (BM=64) fits est_m=139 much better: 2-3 full tiles.
Paired with S2_V1 (64x32, proven for d<2048) and block_m=32 (crash-safe).

d<2048: same as current best (S1_64 or S1_256 based on est_m).
"""
import torch
import functools
    ...
    def new_g2s(token, model_dim, inter_dim, expert, topk,
        ...
        if expert <= 64 and q_type == QuantType.per_1x32 and not r.run_1stage:
            ...
            if not kw.get('kernelName', ''):
                est_m = token * topk // expert
                if inter_dim >= 2048:
                    # d=2048: use SMALL S1_64 (BM=64 fits est_m=139 better)
                    # + S2_V1 (proven) + block_m=32 (crash-safe)

...
#!POPCORN gpu MI355X
"""
GEMM — Extend fused kernel to K<=2048 with BLOCK_K=256.
For K=7168, still use separate path. For K=1536/2048, fused may be faster.
"""
...
@triton.autotune(
    configs=[
        ...
        # BLOCK_K=256 for larger K
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 256, 'GROUP_SIZE_M': 4, 'waves_per_eu': 2, 'matrix_instr_nonkdim': 16}, num_warps=4, num_stages=2),
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 128, 'BLOCK_K': 256, 'GROUP_SIZE_M': 4, 'waves_per_eu': 2, 'matrix_instr_nonkdim': 16}, num_warps=4, num_stages=2),
    ],
    key=['M', 'N', 'K'],
)
@triton.jit

...
#!POPCORN gpu MI355X
"""
BEST COMBINED: Triton env vars + fused custom kernel for K=512 M=32 shapes.
- TRITON_HIP_USE_BLOCK_PINGPONG=1 (better latency hiding)
- TRITON_HIP_USE_ASYNC_COPY=1 (direct-to-LDS loads)
- Fused A-quant+GEMM for M=32 K=512 (1.25x faster)
- aiter defaults for all other shapes
"""
import os
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Fused custom kernel for M=32 K=512 (proven 1.25x faster)
    ...
    # aiter defaults for everything else (with env var optimization)
    if k == 1536:
        from aiter.ops.triton.quant import dynamic_mxfp4_quant

...
#!POPCORN gpu MI355X
"""
MXFP4 GEMM — Expanded AMD-specific Triton compile options sweep.
Testing more waves_per_eu values and kpack.
"""
...
@triton.autotune(
    configs=[
        # Best baseline configs with waves_per_eu sweep
        # waves_per_eu=0 (auto/default)
        ...
        # waves_per_eu=1 (max registers per wave)
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 1, 'waves_per_eu': 1}, num_warps=8, num_stages=2),
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 4, 'waves_per_eu': 1}, num_warps=4, num_stages=2),
        # waves_per_eu=2
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 1, 'waves_per_eu': 2}, num_warps=8, num_stages=2),
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 4, 'waves_per_eu': 2}, num_warps=4, num_stages=2),
        # waves_per_eu=3
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 1, 'waves_per_eu': 3}, num_warps=8, num_stages=2),
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 4, 'waves_per_eu': 3}, num_warps=4, num_stages=2),
        # waves_per_eu=4
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 1, 'waves_per_eu': 4}, num_warps=8, num_stages=2),
        # matrix_instr_nonkdim=16
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 1, 'matrix_instr_nonkdim': 16}, num_warps=8, num_stages=2),
        # Combined: best occupancy + tile size
        ...
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 1, 'waves_per_eu': 1, 'matrix_instr_nonkdim': 16}, num_warps=8, num_stages=2),
        # kpack (AMD K-packing for MFMA)
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 1, 'kpack': 2}, num_warps=8, num_stages=2),
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 1, 'waves_per_eu': 2, 'kpack': 2}, num_warps=8, num_stages=2),
        # Larger tiles with AMD opts
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 128, 'BLOCK_K': 128, 'GROUP_SIZE_M': 4, 'waves_per_eu': 2}, num_warps=4, num_stages=2),

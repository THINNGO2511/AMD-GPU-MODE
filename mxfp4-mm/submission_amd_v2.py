...
#!POPCORN gpu MI355X
"""
MXFP4 GEMM v2 — Sweep-informed configs: BM=64, GS=8 dominant + AMD compile opts.
"""
...
@triton.autotune(
    configs=[
        # Sweep-winning configs: BM=64, GS=8, with AMD opts
        triton.Config({'BLOCK_M': 64, 'BLOCK_N': 128, 'BLOCK_K': 128, 'GROUP_SIZE_M': 8, 'waves_per_eu': 2, 'matrix_instr_nonkdim': 16}, num_warps=8, num_stages=2),
        ...
        triton.Config({'BLOCK_M': 64, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 8, 'waves_per_eu': 2}, num_warps=8, num_stages=2),
        triton.Config({'BLOCK_M': 64, 'BLOCK_N': 64, 'BLOCK_K': 128, 'GROUP_SIZE_M': 8}, num_warps=8, num_stages=2),
        triton.Config({'BLOCK_M': 64, 'BLOCK_N': 256, 'BLOCK_K': 128, 'GROUP_SIZE_M': 8, 'waves_per_eu': 2}, num_warps=4, num_stages=2),
        ...
        # BM=32 configs for small M (M=4, M=16)
        ...
        triton.Config({'BLOCK_M': 32, 'BLOCK_N': 128, 'BLOCK_K': 128, 'GROUP_SIZE_M': 8, 'waves_per_eu': 2}, num_warps=8, num_stages=2),
    ],
    key=['M', 'N', 'K'],
)
@triton.jit

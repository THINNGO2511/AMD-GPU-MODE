...
#!POPCORN gpu MI355X
"""
GEMM — Inject AMD compile opts into gemm_afp4wfp4's autotune configs.
The separate path (K>1024) uses gemm_afp4wfp4 which has its own Triton autotune.
Monkey-patch to add waves_per_eu and matrix_instr_nonkdim.
"""
...
def _patch_gemm_configs():
    """Inject AMD compile opts into gemm_afp4wfp4's internal kernel configs."""
    global _patched_gemm
    if _patched_gemm:
        return
    _patched_gemm = True

    # Find the triton kernel in the module and patch its configs
    try:
        # The kernel is _gemm_afp4wfp4 in the module
        kernel = getattr(afp4_mod, '_gemm_afp4wfp4', None)
        if kernel is None:
            for name in dir(afp4_mod):
                obj = getattr(afp4_mod, name)
                if hasattr(obj, 'configs'):
                    kernel = obj
                    print(f"[PATCH] Found kernel: {name}")
                    break

        if kernel and hasattr(kernel, 'configs'):
            print(f"[PATCH] Original configs: {len(kernel.configs)}")
            # Add AMD opts to each config
            new_configs = []
            for cfg in kernel.configs:
                # Clone config with AMD opts
                kwargs = dict(cfg.kwargs)
                kwargs['waves_per_eu'] = 2
                kwargs['matrix_instr_nonkdim'] = 16
                new_cfg = triton.Config(kwargs, num_warps=cfg.num_warps, num_stages=cfg.num_stages)
                new_configs.append(new_cfg)
            # Add new configs alongside originals
            kernel.configs = list(kernel.configs) + new_configs
            print(f"[PATCH] New configs: {len(kernel.configs)}")

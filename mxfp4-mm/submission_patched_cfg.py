...
#!POPCORN gpu MI355X
"""
GEMM — Monkey-patch _get_config for per-size optimized Triton configs.
Probe what configs _get_config returns by default, then try better ones.
"""
...
def _patch():
    ...
    orig_get_config = afp4_mod._get_config

    def new_get_config(M, N, K, shuffle=False):
        # Get original config
        cfg = orig_get_config(M, N, K, shuffle)

        # Log and try to improve
        print(f"[CFG] M={M} N={N} K={K}: {cfg}")

        # Override with better configs for our specific sizes
        # The default might use suboptimal tile sizes
        # Key insight from top competitors: GROUP_SIZE_M=8 with waves_per_eu=2
        return cfg

    afp4_mod._get_config = new_get_config
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if k <= 1024:
        ...
        _fused_quant_gemm[grid](A, _bq_u8, C, _bscale_raw, m, n, k,

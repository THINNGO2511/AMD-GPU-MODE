...
#!POPCORN gpu MI355X
"""
HIP GEMM kernel v3 for MXFP4 on MI355X (gfx950).
Uses 16x16x128 MFMA FP4 with fused bf16->fp4 A quantization.
Fixed: operand type is int8v (ext_vector_type(8) = 8x int32 = 256 bits = 64 FP4).
...
// The MFMA FP4 intrinsic requires 256-bit operands = 8 x int32
...
// LDS layout (unchanged: 32 rows x 64 fp4 bytes per row)
...
//
// MFMA 16x16x128 FP4:
//   - K=128 FP4 per row, stored as 64 fp4x2 bytes
//   - Operand type: int8v = 8 x int32 = 256 bits = 64 FP4 values per thread
//   - Lane mapping: lr = lane%16 = row/col index, lg = lane/16 = K-group (0..3)
//   - Each lg group provides 64 FP4 from its 256-bit register
//   - Hardware selects the correct K range from each thread's register
//   - lg=0,2 load bytes [0..31], lg=1,3 load bytes [32..63] of each row
    ...
    int M, int N, int K)
{
    ...
    // Accumulators: 16x16 MFMA produces 4 fp32 per thread
    ...
    for (int k_iter = 0; k_iter < K; k_iter += TK) {
        ...
        // Each thread loads 32 bytes (256 bits) = int8v for the MFMA operand.
        // Row is selected by lr (lane%16), K-half by (lg%2):
        //   lg=0,2 -> bytes [0..31] of the row
        //   lg=1,3 -> bytes [32..63] of the row
        int8v a_op;
        {
            int a_row = wm * 16 + lr;
            int a_byte_off = (lg & 1) * 32;   // 0 or 32

...
#!POPCORN gpu MI355X
"""
MXFP4 GEMM — Hybrid optimized: minimal Python overhead.

- K≤1024: Fused quant+GEMM (single kernel via _mxfp4_quant_op + tl.dot_scaled)
- K>1024: Separate quant + gemm_afp4wfp4 (tuned Triton GEMM)
- Tuple-based caching (no dict lookups)
...
# Global cache — tuples not dicts for speed
_bscale_ref = None
_bscale_raw = None
_bq_u8 = None
_out = None
...
def custom_kernel(data: input_t) -> output_t:
    global _bscale_ref, _bscale_raw, _bq_u8, _out, _out_shape

    A, B, B_q, B_shuffle, B_scale_sh = data
    m, k = A.shape
    n = B.shape[0]

    # Cache B data (tuple-based, no dict)
    ...
    shape = (m, n)
    if _out_shape != shape:
        _out = torch.empty(shape, dtype=torch.bfloat16, device='cuda')
        _out_shape = shape
    ...
    if k <= 1024:
        ...
        _fused_quant_gemm[grid](
            A, _bq_u8, _out, _bscale_raw,
            m, n, k,
            A.stride(0), A.stride(1),
            _bq_u8.stride(1), _bq_u8.stride(0),
            _out.stride(0), _out.stride(1),
            _bscale_raw.stride(0), _bscale_raw.stride(1),
            BLOCK_M=BLOCK_M, BLOCK_N=128, BLOCK_K=128, GROUP_SIZE_M=4,

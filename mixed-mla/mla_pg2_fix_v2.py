...
#!POPCORN gpu MI355X
"""
MLA — pg2 fix attempt 2: match reference parameters.

The reference uses:
  kv_granularity = max(PAGE_SIZE, 16) = 16
  NUM_KV_SPLITS = 32

Our pg2 uses:
  kv_granularity = max(1, 16 // page_size) = 8  (DIFFERENT)
  num_kv_splits = 8-16  (DIFFERENT)

This keeps our divided kv_indptr approach but matches the reference's
kv_granularity=16 and num_kv_splits=32.

If the mismatch is from kv_granularity (affecting page boundary handling),
this fixes it. If it's from kv_indptr division, this won't help.
"""
import torch
import triton
...
def _build_meta(batch_size, kv_seq_len, q_seq_len, nq, nkv,
    ...
    # FIX: use reference formula for kv_granularity
    kv_gran = max(page_size, 16)  # = 16 for pg2 (was 8)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # FIX: use num_kv_splits=32 matching reference
    num_kv_splits = 32
    ...
    alloc_key = ("fp8pg2v2", q.shape[0], nq, dv, dq)

...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Conservative: pg2+bf16Q ONLY for bs<=32 kv<=1024 (proven safe on 2/3 secret seeds)
    # pg1+bf16Q for bs>=64 kv<=1024 (avoids pg2 accuracy risk)
    # pg8+fp8Q for kv>=8192 (proven)
    if kv_seq_len <= 1024:
        if batch_size <= 32:
            page_size = 2
            dtype_q = BF16

...
#!POPCORN gpu MI355X
"""
MLA — bf16 KV for small kv, fp8 KV for large kv.

Strategy:
- kv<=1024: pg2 + bf16 Q + bf16 KV (a16w16 kernel)
  * Zero quantization noise on both Q and KV → best accuracy for pg2
  * No fp8 quant kernel launches needed
  * KV bandwidth: bs*1024*576*2 bytes = small (max ~300KB for bs=256)
  * Uses kv_data["bf16"] directly
- kv>=8192: pg8 + fp8 Q + fp8 KV (a8w8 kernel)
  * fp8 saves 2x bandwidth on large KV, essential for speed
...
def _build_meta(batch_size, kv_seq_len, q_seq_len, nq, nkv,
    ...
    """Build metadata for persistent MLA decode. dtype_kv can be BF16 or FP8."""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # kv>=8192: pg8 + fp8 Q + fp8 KV (a8w8 kernel, bandwidth savings)
    if kv_seq_len <= 1024:
        page_size = 2
        dtype_q = BF16
        dtype_kv = BF16
    ...
    if use_bf16_kv:
        # bf16 KV path: kv_data["bf16"] is a plain tensor (total_kv, 1, 576)
        ...
        # bf16 Q + bf16 KV: no quant needed, no scales needed
        mla_decode_fwd(
            q, kv_buffer_4d, o,
        ...
        # fp8 KV path: kv_data["fp8"] is (tensor, scale)

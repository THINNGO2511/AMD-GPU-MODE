...
#!POPCORN gpu MI355X
"""
MLA — fp8 Q + pg2 for ALL sizes with CORRECT kv_granularity.
This is what #1 competitor (Ananda Sai A, 33μs) likely does.

Previous pg2 attempts failed because kv_granularity was wrong:
- WRONG: kv_granularity = max(page_size, 16) = 16
- CORRECT: kv_granularity = max(1, 16 // page_size) = 8  (from PR #1950)

fp8 Q everywhere = minimal bandwidth, pg2 everywhere = halved pages.
Simpler than hybrid (no code path split).
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if cache_key not in _meta_cache:
        ...
        get_mla_metadata_v1(
            ...
            page_size=PAGE_SIZE,
            kv_granularity=max(1, 16 // PAGE_SIZE),  # = 8, THE FIX

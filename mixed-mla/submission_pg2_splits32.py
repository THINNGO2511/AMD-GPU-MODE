...
#!POPCORN gpu MI355X
"""
MLA a16w8+pg2 for kv=1024, a8w8+pg8 for kv=8192.
Competitor "pg2_fix" at 33μs uses pg2 for all sizes.
pg2 for kv=1024 is risky (4.1% mismatch on some seeds) but
the correct kv_granularity formula (max(1, 16//ps)=8) may help.
Key: a16w8 avoids fp8 quant overhead for kv=1024.
Per-shape num_kv_splits: bs=4→4, bs=32→8, bs=64/256→16.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # a16w8+pg2 for kv=1024, a8w8+pg8 for kv=8192
    if kv_seq_len <= 1024:
        page_size = 2  # pg2 instead of pg1 (competitor approach)
        dtype_q = BF16
        use_fp8_q = False
    else:
        page_size = 8
    ...
    # Per-shape num_kv_splits (from pg16_v1 tuning)
    if batch_size <= 4: # pg2 splits32
        num_kv_splits = 32
    elif batch_size <= 32:
        num_kv_splits = 32
    else:

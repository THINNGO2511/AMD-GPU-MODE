...
#!POPCORN gpu MI355X
"""
MLA: Aggressive qseqlen2 — use qseqlen=2 for ALL kv>=8192 shapes.
If this passes accuracy, it could be a major speedup across all large-kv cases.

Risk: qseqlen2 was only validated for bs=256 kv=8192. This tests whether
bs=4/32/64 with kv=8192 also work. If accuracy fails, fall back to
sub_pg2bf16_qseqlen2.py (conservative: only bs>=256).
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        dtype_q = FP8_DTYPE
        use_fp8_q = True
        # AGGRESSIVE: qseqlen2 for ALL kv>=8192 shapes
        max_seqlen_hint = 2

...
#!POPCORN gpu MI355X
"""
MLA: pg4 for kv=8192 (test if faster than pg8), pg2+bf16Q for kv<=1024.
pg4 gives kv_granularity=4, potentially better cache utilization.
pg8 for kv=8192 gives ~3% mismatch — pg4 should be similar or better.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # pg2+bf16Q for kv<=1024, pg4+fp8Q for kv>=8192
    if kv_seq_len <= 1024:
        page_size = 2
        dtype_q = BF16
        use_fp8_q = False
    else:
        page_size = 4  # Try pg4 instead of pg8
        dtype_q = FP8_DTYPE
        use_fp8_q = True

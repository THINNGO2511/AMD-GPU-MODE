...
#!POPCORN gpu MI355X
"""
MLA: pg2+pg8 with Triton BLOCK_PINGPONG (like willfisher's "pingpong.py" at 35μs).
Also: improved splits (4→8 for bs≤4) from Session 9 sweep.
The PINGPONG env var affects the Triton MLA reduce kernel.
"""
import os
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # pg2 for kv<=1024, pg8 for kv>=8192
    if kv_seq_len <= 1024:
        page_size = 2
        dtype_q = BF16
        use_fp8_q = False
    ...
    # Optimal splits (fixed: 8 for bs<=4, not 4)
    if batch_size <= 4:
        num_kv_splits = 8
    elif batch_size <= 32:
        num_kv_splits = 8

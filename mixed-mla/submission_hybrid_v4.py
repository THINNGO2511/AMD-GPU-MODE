...
#!POPCORN gpu MI355X
"""
MLA — Hybrid v4: pg2 everywhere, switch Q type by batch size:
- bs<=64: a16w8+pg2 (bf16 Q, no quant, pg2 speedup, accuracy safe)
- bs=256: a8w8+pg2 (fp8 Q saves bandwidth on large batch, pg2 speedup)
All with page_size=2. Direct stage1+reduce calls.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # a16w8 for small batch (Q quant overhead > bandwidth savings)
    # a8w8 for large batch (fp8 Q saves bandwidth)
    use_a16w8 = batch_size <= 64

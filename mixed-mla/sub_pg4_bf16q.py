...
#!POPCORN gpu MI355X
"""
MLA — pg4+bf16Q for kv≤1024 (instead of pg2). pg8+fp8Q for kv≥8192.
pg2 fails 33% on bs=64 kv=1024 secret seed.
pg4 with bf16 Q might have better accuracy → higher pass rate.
pg4 = 256 pages for kv=1024 (vs pg2=512, pg1=1024, pg8=128).
kv_granularity = max(1, 16//4) = 4.
"""
import os
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # pg4+bf16Q for kv≤1024, pg8+fp8Q for kv≥8192
    if kv_seq_len <= 1024:
        page_size = 4      # pg4 instead of pg2
        dtype_q = BF16
        use_fp8_q = False
    else:
        page_size = 8

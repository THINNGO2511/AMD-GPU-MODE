...
#!POPCORN gpu MI355X
"""
MLA — Direct stage1+reduce. Mixed page_size:
- kv=1024: page_size=1 (safe accuracy, ~4% mismatch was too close to 5% with pg2)
- kv=8192: page_size=2 (faster, mismatch ~1.4% is safely under 5%)
Pre-allocate all intermediates. Fused Triton Q quant.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Mixed page_size: pg1 for kv=1024 (accuracy), pg2 for kv=8192 (speed)
    if kv_seq_len <= 1024:
        PAGE_SIZE = 1
    else:
        PAGE_SIZE = 2

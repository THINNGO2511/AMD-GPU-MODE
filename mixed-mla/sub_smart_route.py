...
#!POPCORN gpu MI355X
"""
MLA Smart Route — per-shape optimal (page_size, Q_dtype) routing for 100% pass rate.

Routing table:
  bs<=32  kv<=1024 : pg2 + bf16Q  (fast, proven safe on all seeds)
  bs>=64  kv<=1024 : pg1 + bf16Q  (safe, avoids pg2 ~4.1% mismatch risk at large bs)
  ALL     kv>=8192 : pg8 + fp8Q   (proven fast + safe, ~1.4% mismatch max)

Expected geomean ~39us with 100% pass rate vs ~40us with 67% pass rate (pg2-all).

Key formulas:
  kv_granularity = max(1, 16 // page_size)   [PR #1950]
  kv_indptr_pages = kv_indptr // page_size    [page-level indptr]
  kv_last_page_len = seq_lens % ps, 0->ps     [last page handling]
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # ---- Smart routing ----
    # kv>=8192: pg8 + fp8Q (bandwidth-bound, pg8 halves page overhead, fp8 halves Q bandwidth)
    # bs<=32 kv<=1024: pg2 + bf16Q (compute-light, pg2 safe at small bs, bf16Q avoids quant cost)
    # bs>=64 kv<=1024: pg1 + bf16Q (pg2 has ~4.1% mismatch at large bs, too close to 5% threshold)
    if kv_seq_len >= 8192:
        page_size = 8
        dtype_q = FP8_DTYPE
        use_fp8_q = True
    ...
    if use_fp8_q:
        # fp8 Q path: quantize Q to fp8, use a8w8 ASM kernel
        ...
        # bf16 Q path: pass Q directly, use a16w8 ASM kernel (no quant overhead)

...
#!POPCORN gpu MI355X
"""
MLA — fp8 Q + fp8 KV + page_size=2 + kv_granularity=2.
Attempt to fix pg2 accuracy by matching kv_granularity to page_size.
With kv_granularity=2, the kernel processes exactly 1 page (2 tokens) per chunk,
which should preserve the same accumulation pattern as pg1 with kv_granularity=16
processing 16 tokens per chunk.
Also try num_kv_splits=4 for less reduction error.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Fewer splits to reduce split-reduce error with pg2
    if num_pages <= 2048:
        num_kv_splits = 4
    elif num_pages <= 8192:
        num_kv_splits = 8
    ...
    if cache_key not in _meta_cache:
        ...
        get_mla_metadata_v1(
            ...
            page_size=PAGE_SIZE,
            kv_granularity=PAGE_SIZE,  # Key: match granularity to page_size

...
#!POPCORN gpu MI355X
"""
MLA Decode — Fused Q fp8 quant + splits=16 + AMD compile options on Triton kernels.
Test waves_per_eu on Q quantization kernels.
"""
import torch
import triton
...
@triton.jit
def _q_amax_kernel(q_ptr, amax_ptr, N, BLOCK: tl.constexpr, waves_per_eu: tl.constexpr):
    pid = tl.program_id(0)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    amax_buf.zero_()
    _q_amax_kernel[grid](q, amax_buf, N, BLOCK=BLOCK, waves_per_eu=2)
    _q_to_fp8_kernel[grid](q, q_fp8_flat, scale_buf, amax_buf,
                           FP8_MAX=_FP8_MAX, N=N, BLOCK=BLOCK, waves_per_eu=2)

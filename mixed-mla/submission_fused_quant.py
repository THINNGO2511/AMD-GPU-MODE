...
@triton.jit
def _fused_q_fp8_kernel(q_ptr, out_ptr, scale_ptr,
                        stride_row: tl.int64, N_cols,
                        FP8_MAX: tl.constexpr, BLOCK_N: tl.constexpr):
    """Fused per-tensor amax + fp8 cast in one kernel. One program per row."""
    row = tl.program_id(0)
    offs = tl.arange(0, BLOCK_N)
    mask = offs < N_cols
    x = tl.load(q_ptr + row * stride_row + offs, mask=mask, other=0.0).to(tl.float32)
    # Per-row amax (will be reduced across rows via atomic)
    local_amax = tl.max(tl.abs(x))
    tl.atomic_max(scale_ptr, local_amax)
    # Sync not needed — we read scale_ptr after ALL programs write
    # BUT: we need a global barrier which Triton doesn't have.
    # Fallback: two-pass is unavoidable for per-tensor quant.
    # Keep the original two-kernel approach but skip the zero_() call.
    pass

# Actually, keep the proven two-kernel approach — the zero_() is negligible.
# The real overhead saving is pre-allocating everything and minimizing Python.
@triton.jit

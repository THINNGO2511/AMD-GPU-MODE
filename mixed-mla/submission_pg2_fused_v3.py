...
#!POPCORN gpu MI355X
"""
MLA — fp8 Q + fp8 KV + page_size=2 with fused Q quant + per-size splits.
TOLERANCE LOOSENED to rtol=0.1 atol=0.1 (PR #137, Mar 18) + 5% mismatch bypass.
Our pg2 error (0.089-0.095) was under OLD rtol=0.01. New rtol=0.1 should pass.
Combines best of fused_v2 (Q quant kernels) with pg2 (28% speedup).
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Per-size tuned splits (same logic as our best pg1 fused_v2)
    if total_kv <= 8192:
        num_kv_splits = 8
    else:
        num_kv_splits = 16
    ...
    kv_buffer_4d = kv_buffer_fp8.view(
        -1, PAGE_SIZE, nkv, kv_buffer_fp8.shape[-1]
    )

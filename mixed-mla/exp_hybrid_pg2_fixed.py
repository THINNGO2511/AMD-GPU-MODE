"""
MLA Experiment: Hybrid a16w8+pg2 (kv<=1024) / a8w8+pg2 (kv>=8192).
Key fix: correct kv_granularity = max(1, 16//page_size) = 8 for pg2.
This combines: no fp8 quant overhead for small KV + fp8 bandwidth savings for large KV.
Both paths use pg2 for reduced page count.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Hybrid: a16w8 for small KV (no quant overhead), a8w8 for large KV (bandwidth)
    use_fp8_q = kv_seq_len > 1024

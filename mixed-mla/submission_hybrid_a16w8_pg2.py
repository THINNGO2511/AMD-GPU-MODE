...
"""
MLA — Optimal hybrid:
- kv<=1024: a16w8 (bf16 Q + fp8 KV, pg1) — skip Q quant, saves ~5-10%
- kv>=8192: a8w8 (fp8 Q + fp8 KV, pg2) — 28% faster on large kv
Separate metadata caches per (dtype_q, page_size) combo.
Pre-allocate all intermediates. Fused Triton Q quant for fp8 path.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    cache_key = (batch_size, kv_seq_len, num_kv_splits, PAGE_SIZE, dtype_q)
    if cache_key not in _meta_cache:
        ...
        _meta_cache[cache_key] = (
            ...
            logits, attn_lse, PAGE_SIZE,
        )

    (work_metadata, work_indptr, work_info_set,
     reduce_indptr, reduce_final_map, reduce_partial_map,
     kv_indices, kv_last_page_len, kv_indptr_pages,
     logits, attn_lse, PAGE_SIZE) = _meta_cache[cache_key]

    kv_buffer_fp8, kv_scale = kv_data["fp8"]
    kv_buffer_4d = kv_buffer_fp8.view(-1, PAGE_SIZE, nkv, kv_buffer_fp8.shape[-1])

    if use_a16w8:
        # a16w8 path: bf16 Q directly, no quantization
        ...
        # a8w8 path: quantize Q to fp8, use pg2

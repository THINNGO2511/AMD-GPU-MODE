...
#!POPCORN gpu MI355X
"""
MLA pg8 ALL — page_size=8 for ALL kv sizes.
pg8_v2 uses pg1+bf16Q for kv≤1024, pg8+fp8Q for kv≥8192.
This tests pg8+bf16Q for kv≤1024 too (no fp8 quant, but faster paging).
kv_granularity = max(1, 16 // 8) = 2 for all cases.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    page_size = 8  # pg8 for ALL sizes

    # fp8 Q for kv>=8192 (bandwidth-bound), bf16 Q for kv<=1024 (compute-bound)
    if kv_seq_len <= 1024:
        dtype_q = BF16
        use_fp8_q = False
    else:
    ...
    # Per-shape num_kv_splits tuning
    if batch_size <= 4:
        num_kv_splits = 4
    elif batch_size <= 32:
        num_kv_splits = 8

...
#!POPCORN gpu MI355X
"""
MLA — pg2+bf16Q with kv_granularity=16 (reference formula, NOT PR #1950).
UNTESTED combination! Our pg2 uses gran=8 (max(1,16//2)), reference uses gran=16 (max(ps,16)).
Using gran=16 with pg2 matches reference scheduling while keeping page-count benefit.
This might be what "pg2_fix for all sizes" means — the competitor's fix.
"""
import os
...
def _build_meta(batch_size, kv_seq_len, q_seq_len, nq, nkv,
    ...
    # KEY CHANGE: use reference formula max(PAGE_SIZE, 16) = 16
    # instead of PR #1950 formula max(1, 16 // page_size) = 8
    kv_gran = max(page_size, 16)  # = 16 for pg2, = 16 for pg8

...
#!POPCORN gpu MI355X
"""
MLA: pg1+bf16Q for kv<=1024 (100% safe), pg8+fp8Q+qseqlen2 for bs>=256 kv>=8192.
pg2 for kv=1024 FAILED leaderboard (26706 mismatches on secret seed bs=64).
pg1 is slower but 100% reliable.
qseqlen2 gives 49% speedup on bs=256 kv=8192 (96.3μs vs ~189μs).
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        # pg1 + bf16Q — 100% reliable (pg2 fails on some secret seeds)
        page_size = 1
        dtype_q = BF16
        use_fp8_q = False
        max_seqlen_hint = q_seq_len
    else:
        # pg8 + fp8Q + qseqlen2 for large batches
        page_size = 8
        dtype_q = FP8_DTYPE
        use_fp8_q = True
        if batch_size >= 256:

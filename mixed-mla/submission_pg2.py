...
#!POPCORN gpu MI355X
"""
MLA — page_size=2 (top competitor uses "pg2_fix" at 33μs).
Previous attempt with page_size>1 failed correctness.
Fix: adjust kv_granularity to match page_size, and use page_size for KV buffer view.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if cache_key not in _meta_cache:
        ...
        # For page_size=2, each "page" has 2 KV entries
        num_pages = (total_kv_len + PAGE_SIZE - 1) // PAGE_SIZE
        # kv_indptr needs to be in pages, not tokens
        kv_indptr_pages = kv_indptr // PAGE_SIZE
        kv_last_page_len = torch.full((batch_size,), PAGE_SIZE, dtype=torch.int32, device='cuda')
        # Adjust last page length for each batch
    ...
    # Reshape KV for page_size=2: [num_pages, PAGE_SIZE, nkv, head_dim]

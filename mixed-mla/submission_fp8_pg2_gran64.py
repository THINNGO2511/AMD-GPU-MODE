...
#!POPCORN gpu MI355X
"""
MLA — fp8 Q + fp8 KV + page_size=2 + kv_granularity=64.
Research suggests larger kv_granularity reduces split boundary errors
that cause pg2 accuracy failure. With granularity=64, fewer split
boundaries means less accumulated error from the kv_offset formula.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # With larger granularity, use fewer splits
    if total_kv <= 8192:
        num_kv_splits = 8
    else:
        num_kv_splits = 16
    ...
    if cache_key not in _meta_cache:
        ...
        get_mla_metadata_v1(
            ...
            page_size=PAGE_SIZE,
            kv_granularity=64,  # Large granularity to reduce split errors

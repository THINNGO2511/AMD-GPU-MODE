...
#!POPCORN gpu MI355X
"""
MLA pg8 v3 micro — micro-optimized pg1+pg8 path.
Changes from v2:
- Per-shape num_kv_splits tuning (8/12/16 instead of fixed 8/16)
- Smaller BLOCK=2048 for Q quant (better for small batch)
- Cached kv_buffer_4d views
- Pre-allocated zero buffer for amax
"""
import torch
import triton
...
# Splits: match proven v2 logic (8 for small, 16 for large)
def _get_splits(batch_size, kv_seq_len):
    total_kv = batch_size * kv_seq_len
    return 8 if total_kv <= 8192 else 16
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Cache kv_buffer_4d view
    kv_view_key = (kv_buffer_fp8.data_ptr(), ps, nkv)
    if kv_view_key not in _kv_view_cache:
        _kv_view_cache[kv_view_key] = kv_buffer_fp8.view(-1, ps, nkv, kv_buffer_fp8.shape[-1])
    kv_buffer_4d = _kv_view_cache[kv_view_key]

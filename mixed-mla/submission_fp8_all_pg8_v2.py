...
#!POPCORN gpu MI355X
"""
MLA fp8 Q for ALL sizes + pg1 for kv=1024, pg8 for kv=8192.
Current best uses bf16 Q for kv=1024 (avoids quant).
This tests fp8 Q everywhere — may help if quant overhead < bandwidth savings.
fp8 Q saves 50% Q memory but adds 2 Triton kernel launches.
For bs=256, kv=1024: Q is 256*16*576*2=4.7MB bf16 vs 2.4MB fp8.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # fp8 Q for ALL + pg1 for kv=1024, pg8 for kv=8192
    if kv_seq_len <= 1024:
        page_size = 1
    else:
        page_size = 8

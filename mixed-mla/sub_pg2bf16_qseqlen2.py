...
#!POPCORN gpu MI355X
"""
MLA: pg2+bf16Q for kv<=1024, pg8+fp8Q for kv>=8192, PLUS qseqlen=2
dispatch for bs>=256 kv>=8192 (proven 10% faster on runner, passed accuracy).

Combines the proven sub_pg2_bf16q base with the qseqlen2 dispatch trick:
  max_seqlen_hint=2 tells the ASM kernel to process 2 Q tokens together,
  improving occupancy for large batches. Only safe for bs>=256 kv>=8192.
"""
import torch
import triton
...
def _build_meta(batch_size, kv_seq_len, q_seq_len, nq, nkv,
    ...
    return (wm, wi, wis, ri, rfm, rpm, kv_indices, kv_last_page_len,
            kv_indptr_pages, page_size, max_seqlen_hint)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        dtype_q = FP8_DTYPE
        use_fp8_q = True
        # qseqlen2 dispatch for bs>=256 with kv>=8192 (proven safe + 10% faster)
        if batch_size >= 256:
            max_seqlen_hint = 2
        else:
            max_seqlen_hint = q_seq_len
    ...
    cache_key = (batch_size, kv_seq_len, num_kv_splits, page_size, use_fp8_q, max_seqlen_hint)

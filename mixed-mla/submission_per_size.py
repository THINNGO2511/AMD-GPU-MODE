...
#!POPCORN gpu MI355X
"""
MLA Decode — Per-size tuned num_kv_splits + fused Q fp8 quant.
Try different splits for different (batch_size, kv_seq_len) combinations.
Also try passing mxfp4 KV to mla_decode_fwd.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if not _probed:
        _probed = True
        # Probe: what keys does kv_data have?
        print(f"[PROBE] kv_data keys: {list(kv_data.keys())}")
        ...
        # Try passing mxfp4 KV to mla_decode_fwd
        import inspect
        sig = inspect.signature(mla_decode_fwd)
        print(f"\n[PROBE] mla_decode_fwd params:")
        for name, p in sig.parameters.items():
            print(f"  {name}: {p.default if p.default != inspect.Parameter.empty else 'required'}")

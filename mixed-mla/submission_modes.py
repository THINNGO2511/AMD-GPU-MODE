...
#!POPCORN gpu MI355X
"""
MLA — Try fast_mode=True + per-size splits.
Previously fast_mode was 5-10% worse with splits=16, but maybe different with 8.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if total_kv <= 8192:
        num_kv_splits = 8
        use_fast = True  # Try fast_mode for small cases
    else:
        num_kv_splits = 16
        use_fast = False

    cache_key = (batch_size, kv_seq_len, num_kv_splits, use_fast)
    if cache_key not in _meta_cache:
        ...
        info = get_mla_metadata_info_v1(
            ...
            is_sparse=False, fast_mode=use_fast,

...
#!POPCORN gpu MI355X
"""
MLA pg8 v3 — pg1+bf16Q for kv<=1024, pg8+fp8Q for kv>=8192.
Improvements over v2:
- num_kv_splits=16 for ALL shapes (sweep shows 16 optimal everywhere)
- Fused Q fp8 quant with larger BLOCK for small tensors
- Pre-allocate ALL output tensors on first call
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # pg1+bf16Q for kv<=1024 (safe accuracy), pg8+fp8Q for kv>=8192
    if kv_seq_len <= 1024:
        page_size = 1
        dtype_q = BF16
        use_fp8_q = False
    ...
    # Optimal splits from Session 9 sweep: 16 is best or tied for ALL shapes
    # Exception: bs=4 kv=1024 where splits=8 ties with 16 (negligible)
    num_kv_splits = 16

...
#!POPCORN gpu MI355X
"""
MLA pg16/pg4 — push page sizes further.
pg16 for kv>=8192 (kv_gran=1), pg4 for kv<=1024 (kv_gran=4).
Per-shape num_kv_splits tuning: bs=4→4, bs=32→8, bs=64→16, bs=256→16.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Route: pg4+bf16Q for kv<=1024, pg16+fp8Q for kv>=8192
    if kv_seq_len <= 1024:
        page_size = 4
        dtype_q = BF16
        use_fp8_q = False

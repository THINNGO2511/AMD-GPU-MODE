...
#!POPCORN gpu MI355X
"""
MLA — Fine-grained per-case num_kv_splits selection.
Benchmark cases (total_kv):
  bs=4  kv=1k:    4096  → try 8
  bs=4  kv=8k:   32768  → try 16
  bs=32 kv=1k:   32768  → try 8 or 16
  bs=32 kv=8k:  262144  → try 16
  bs=64 kv=1k:   65536  → try 8 or 16
  bs=64 kv=8k:  524288  → try 16
  bs=256 kv=1k: 262144  → try 16
  bs=256 kv=8k: 2M      → try 16 or 32
"""
import torch
import triton
...
def _get_splits(batch_size, kv_seq_len):
    """Per-case optimal splits."""
    total_kv = batch_size * kv_seq_len
    if total_kv <= 8192:
        return 8
    elif total_kv >= 500000:
        return 32  # Try more splits for very large cases
    return 16

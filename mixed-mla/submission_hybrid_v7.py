...
#!POPCORN gpu MI355X
"""
MLA Hybrid v7 — Safe a16w8+pg1 / a8w8+pg2 via mla_decode_fwd (high-level API).
- kv<=1024: bf16 Q + fp8 KV + pg1 → no quant overhead, pg1 safe accuracy
- kv>=8192: fp8 Q + fp8 KV + pg2 → 28% faster, pg2 accuracy safe at ~1.4%
Uses mla_decode_fwd (same JIT as reference) to avoid timeout from extra JIT modules.
Pre-warms Triton Q quant kernels at first call.
"""
import torch
import triton
...
def _build_meta(batch_size, kv_seq_len, q_seq_len, nq, nkv,
                ...
                qo_indptr, kv_indptr):
    """Build metadata for a given (page_size, dtype_q) configuration."""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Route: a16w8+pg1 for small kv, a8w8+pg2 for large kv
    if kv_seq_len <= 1024:
        page_size = 1
        dtype_q = BF16
        use_fp8_q = False
    ...
    if use_fp8_q:
        ...
        # bf16 Q path — no quantization, page_size=1

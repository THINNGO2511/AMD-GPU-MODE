...
#!POPCORN gpu MI355X
"""
MLA — Hybrid: bf16 KV for small cases (skip quant overhead), fp8 for large.
For bs=4 kv=1k, the Q fp8 quant kernels (~3μs) are significant overhead.
Try bf16 Q + bf16 KV path which skips all quantization.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Use bf16 path for small cases (avoid quant overhead)
    use_bf16 = (total_kv <= 4096)
    ...
    cache_key = (batch_size, kv_seq_len, num_kv_splits, use_bf16)
    if cache_key not in _meta_cache:
        total_kv_len = batch_size * kv_seq_len
        kv_last_page_len = (kv_indptr[1:] - kv_indptr[:-1]).to(torch.int32)

        info = get_mla_metadata_info_v1(
            batch_size, q_seq_len, nq, q_dtype, kv_dtype,
    ...
    alloc_key = (q.shape[0], nq, dv, dq, use_bf16)
    ...
    if use_bf16:
        ...
        kv_buffer_4d = kv_buffer_bf16.view(
            kv_buffer_bf16.shape[0], PAGE_SIZE, nkv, kv_buffer_bf16.shape[-1]
        )
        q_input = q  # already bf16
        ...
        # fp8 path: quantize Q
        amax_buf = allocs['amax_buf']
        scale_buf = allocs['scale_buf']
        q_fp8_flat = allocs['q_fp8_flat']

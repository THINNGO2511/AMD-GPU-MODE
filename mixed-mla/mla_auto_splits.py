...
#!POPCORN gpu MI355X
"""
MLA — Auto-tuned num_kv_splits from runner source code.

RUNNER DISCOVERY: mla.py has an auto-tuning formula:
  cu_num = 256  # MI355X
  overhead = 84.1
  score = bs * splits / ceil(bs * splits / cu_num) * avg_kv / (avg_kv + overhead * splits)
  Pick splits with highest score from candidates [1, 2, 4, 8, 16, 32]

Our manual config: bs<=4→4, bs<=64→8, bs>64 kv<=1024→8, else→16.
The formula might give DIFFERENT values for our exact shapes.

Let's compute and use the formula directly.
"""
import torch
import math
import triton
...
def _auto_tune_splits(batch_size, kv_seq_len):
    """Compute optimal num_kv_splits using the runner's formula."""
    avg_kv = kv_seq_len  # uniform seq len in benchmark
    best_score = -1
    best_splits = 8  # fallback

    for splits in [1, 2, 4, 8, 16, 32]:
        total_work = batch_size * splits
        waves = math.ceil(total_work / CU_NUM)
        utilization = total_work / (waves * CU_NUM)
        kv_efficiency = avg_kv / (avg_kv + OVERHEAD * splits)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # AUTO-TUNED SPLITS from runner formula
    num_kv_splits = _auto_tune_splits(batch_size, kv_seq_len)

    if _call <= 8:
        print(f"[MLA] bs={batch_size} kv={kv_seq_len} auto_splits={num_kv_splits}", flush=True)
    ...
    alloc_key = ("auto", q.shape[0], nq, dv, dq)

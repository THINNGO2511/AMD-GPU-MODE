...
#!POPCORN gpu MI355X
"""
MLA Hybrid pg2 v2 — pg2 for ALL sizes, optimal Q dtype per kv_len:
- kv=1024: bf16 Q (a16w8) + pg2 → no quant overhead + page halving
- kv=8192: fp8 Q (a8w8) + pg2 → fp8 saves bandwidth + page halving

v1 used pg1 for kv=1024 which was SLOWER. This keeps pg2 everywhere
(like the successful a16w8_ps2 submission) but adds fp8 Q for kv=8192
...
def _build_pg2_meta(batch_size, kv_seq_len, q_seq_len, nq, nkv,
                    num_kv_splits, dtype_q, qo_indptr, kv_indptr):
    """Build pg2 metadata for a given Q dtype."""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # pg2 for ALL sizes, but optimal Q dtype:
    # kv=1024: bf16 Q (a16w8) — saves 2 quant kernel launches
    # kv=8192: fp8 Q (a8w8) — saves 2x bandwidth on Q
    use_fp8_q = (kv_seq_len >= 8192)
    ...
    cache_key = (batch_size, kv_seq_len, num_kv_splits, use_fp8_q)
    if cache_key not in _meta_cache:
        _meta_cache[cache_key] = _build_pg2_meta(

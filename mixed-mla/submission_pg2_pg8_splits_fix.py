...
#!POPCORN gpu MI355X
"""
MLA — Based on our BEST scoring submission (submission_a16w8_pg2_pg8.py at 42.488μs).
ONLY change: fix num_kv_splits for bs<=4 from 4→8 (Session 9 sweep: 2.6x faster).
Everything else IDENTICAL to the 42.488μs submission.
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # a16w8+pg2 for kv=1024, a8w8+pg8 for kv=8192 (same as 42.488μs best)
    if kv_seq_len <= 1024:
        page_size = 2
        dtype_q = BF16
        use_fp8_q = False
    ...
    # FIXED: optimal num_kv_splits from Session 9 sweep
    # bs=4: splits=4→8 (sweep: 0.102ms→0.039ms = 2.6x faster!)
    # bs=32: splits=8 (same as before, already optimal)
    # bs=64+: splits=16 (same as before, already optimal)
    if batch_size <= 4:
        num_kv_splits = 8   # WAS 4, NOW 8 — the key fix
    elif batch_size <= 32:
        num_kv_splits = 8
    else:
        num_kv_splits = 16

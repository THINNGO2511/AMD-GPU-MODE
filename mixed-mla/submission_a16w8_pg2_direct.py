...
#!POPCORN gpu MI355X
"""
MLA — Pure a16w8 + pg2 for ALL sizes. Direct stage1+reduce calls.
bf16 Q + fp8 KV + page_size=2 everywhere.
bf16 Q avoids fp8 quantization noise → pg2 accuracy may hold for kv=1024 too.
No Q quant kernels needed → saves 2 kernel launches for ALL cases.
"""
import torch
import aiter

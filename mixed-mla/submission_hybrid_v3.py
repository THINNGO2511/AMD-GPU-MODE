...
#!POPCORN gpu MI355X
"""
MLA — Optimal hybrid v3:
- Small batch (bs<=64) + kv<=1024: a16w8+pg1 (skip Q quant, Q is tiny)
- Large batch (bs=256) + kv<=1024: a8w8+pg1 (fp8 Q saves bandwidth)
- All kv>=8192: a8w8+pg2 (28% speedup, ~1.4% mismatch is safe)
Separate metadata caches per (page_size, dtype_q) combo.
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Routing logic:
    # - Small batch + small kv: a16w8 (Q quant overhead > bandwidth savings)
    # - Large batch + small kv: a8w8 (fp8 Q saves bandwidth)
    # - Large kv: a8w8+pg2 (pg2 halves page iterations)
    if kv_seq_len >= 8192:
        PAGE_SIZE = 2
        use_a16w8 = False
    elif batch_size <= 64:

...
#!POPCORN gpu MI355X
"""
MLA pg1+pg8 SAFE approach with maximum optimizations:
- pg1 for kv<=1024 (SAFE, no accuracy risk)
- pg8 for kv>=8192 (SAFE, ~3% mismatch)
- Optimal num_kv_splits=16 for all shapes (Session 9 sweep)
- a16w8 (bf16 Q, no quant) for kv<=1024
- a8w8 (fp8 Q) for kv>=8192
- Fused Q quant (2 Triton kernels)
- All tensors pre-allocated
Targets: beat 42.488μs safely (no pg2 gamble)
"""
import torch
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # SAFE: pg1 for kv<=1024, pg8 for kv>=8192
    if kv_seq_len <= 1024:
        page_size = 1
        dtype_q = BF16
        use_fp8_q = False
    ...
    # Optimal: splits=16 for ALL shapes (Session 9 sweep)
    num_kv_splits = 16

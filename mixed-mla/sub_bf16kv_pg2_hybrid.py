...
#!POPCORN gpu MI355X
"""
MLA — bf16 KV for kv<=1024 + fp8 KV for kv>=8192, bf16 Q throughout.

Strategy:
- kv<=1024: pg2 + bf16 Q + bf16 KV (a16w16 kernel)
  * Zero quantization noise on BOTH Q and KV → best possible pg2 accuracy
  * No fp8 quant kernel launches → fewer kernel dispatches
  * KV bandwidth small: bs*1024*576*2 = max ~300KB for bs=256
  * Uses kv_data["bf16"] directly
- kv>=8192: pg8 + bf16 Q + fp8 KV (a16w8 kernel)
  * fp8 KV saves 2x bandwidth on large KV, essential for speed
  * bf16 Q avoids fp8 Q quant overhead (2 kernel launches saved)
  * a16w8 kernel: bf16 Q + fp8 KV, no q_scale needed
  * pg8 proven safe at ~1.4% mismatch for 8k shapes

Key: kv_granularity = max(1, 16 // page_size) per PR #1950.
"""
import torch
import triton
...
def _build_meta(batch_size, kv_seq_len, q_seq_len, nq, nkv,
    ...
    """Build metadata for persistent MLA decode."""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Route by kv_seq_len:
    # kv<=1024: pg2 + bf16 Q + bf16 KV (a16w16 kernel, zero quant noise)
    # kv>=8192: pg8 + bf16 Q + fp8 KV (a16w8 kernel, bf16 Q saves quant overhead)
    if kv_seq_len <= 1024:
        page_size = 2
        dtype_q = BF16
        dtype_kv = BF16
    ...
    if use_bf16_kv:
        ...
        # kv_data["bf16"] is a plain tensor (total_kv, 1, 576)
        ...
        # bf16 Q + bf16 KV → a16w16 kernel: no scales needed at all
        mla_decode_fwd(
            q, kv_buffer_4d, o,
        ...
        # === fp8 KV path (kv>=8192) ===
        # kv_data["fp8"] is (tensor, scale)
        kv_buffer_fp8, kv_scale = kv_data["fp8"]
        kv_buffer_4d = kv_buffer_fp8.view(-1, ps, nkv, kv_buffer_fp8.shape[-1])

        alloc_key = ("fp8kv_bf16q", q.shape[0], nq, dv)
        if alloc_key not in _alloc_cache:
            _alloc_cache[alloc_key] = torch.empty(
                (q.shape[0], nq, dv), dtype=BF16, device="cuda")
        o = _alloc_cache[alloc_key]

        # bf16 Q + fp8 KV → a16w8 kernel: no q_scale, only kv_scale
        mla_decode_fwd(
            q, kv_buffer_4d, o,
            qo_indptr, kv_indptr_pages, kv_indices, kv_last_page_len,
            q_seq_len, page_size=ps, nhead_kv=nkv,

...
#!POPCORN gpu MI355X
"""
MLA — pg2 everywhere with REDUCED splits for kv=1024 to fix accuracy.
Theory: mismatch ~ C/sqrt(kv/splits). With splits=4 instead of 8,
each partial sum has 256 tokens (not 128) → more stable → ~2.9% mismatch.
kv=8192: splits=16 (proven safe at 1.4%)
kv=1024: splits=4 (predicted ~2.9%, under 5%)
"""
import torch
import triton
...
# Key: fewer splits for kv=1024 to reduce accumulation error
_SPLITS = {
    (4, 1024): 4,     # was 8 → 4 for accuracy
    (32, 1024): 4,    # was 8 → 4 for accuracy
    (64, 1024): 4,    # was 8 → 4 for accuracy
    (256, 1024): 4,   # was 16 → 4 for accuracy
    (4, 8192): 16,
    (32, 8192): 16,
    (64, 8192): 16,
    (256, 8192): 16,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if cache_key not in _meta_cache:
        ...
        num_pages_per_req = seq_lens_kv // PAGE_SIZE
        ...
        kv_last_page_lens = torch.full((batch_size,), PAGE_SIZE, dtype=torch.int32, device="cuda")

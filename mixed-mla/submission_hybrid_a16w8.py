...
#!POPCORN gpu MI355X
"""
MLA Decode — Hybrid a16w8/a8w8 based on problem size.
Small batch: a16w8 (skip Q quant overhead)
Large batch: a8w8 (fp8 compute throughput)
"""
import torch
import triton
...
def _build_metadata(batch_size, kv_seq_len, q_seq_len, nq, nkv, 
                    qo_indptr, kv_indptr, num_kv_splits, q_dtype):
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Decide: a16w8 (bf16 Q) vs a8w8 (fp8 Q)
    # a16w8 saves ~5-10us Q quant but fp8 compute is faster
    # For small cases, quant overhead dominates → a16w8
    # For large cases, compute dominates → a8w8
    use_a16w8 = (total_kv <= 32768)  # bs<=32 or bs=64,kv=1k
    ...
    if cache_key not in _meta_cache:
        _meta_cache[cache_key] = _build_metadata(

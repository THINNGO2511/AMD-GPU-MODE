...
#!POPCORN gpu MI355X
"""
MLA Decode — Single-kernel fused Q fp8 quant + per-size num_kv_splits.
Fuse amax reduction + scale computation + fp8 cast into ONE kernel launch.
"""
import torch
import triton
...
@triton.jit
def _fused_fp8_quant_kernel(
    q_ptr, out_ptr, scale_ptr, amax_ptr,
    FP8_MAX: tl.constexpr, N, BLOCK: tl.constexpr,
    NUM_BLOCKS: tl.constexpr,
):
    """Phase 1: Compute per-block max and atomic_max to global amax."""
    pid = tl.program_id(0)
    ...
    # Barrier: all blocks must finish amax before reading
    # We can't do a global barrier in Triton, so we use a two-pass approach
    # But single-kernel approach: just read amax after store (may be stale)
    # This won't work correctly — need two passes or cooperative groups
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # bs=4 kv=1k: small → 8 splits
    # bs=4 kv=8k: medium → 16 splits
    # bs=32 kv=1k: small → 8 splits
    # bs=32 kv=8k: large → 16 splits
    # bs=64 kv=1k: medium → 8 splits
    # bs=64 kv=8k: large → 16 splits
    # bs=256 kv=1k: large → 16 splits
    # bs=256 kv=8k: very large → 16 splits
